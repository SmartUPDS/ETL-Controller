...
//            this.checkN3Validity();
    ...
    private void checkN3Validity() throws FileNotFoundException{
        ...
        Timer.start(ZeriHomogenizer.class.getCanonicalName()+".n3validation.artworks");
        Utils.N3Validator(new File(Resources.FOLDER_OUTPUT_TRANSFORMED_ZERI_ARTWORKS));
        Timer.stop(ZeriHomogenizer.class.getCanonicalName()+".n3validation.artworks");
        log.info("FINISH: Check the validity of artworks in N3 format from Zeri in "+Timer.reportHumanFriendly(ZeriHomogenizer.class.getCanonicalName()+".n3validation.artworks"));

        log.info("START: Check the validity of photographs in N3 format from Zeri");
        Timer.start(ZeriHomogenizer.class.getCanonicalName()+".n3validation.photographs");
        Utils.N3Validator(new File(Resources.FOLDER_OUTPUT_TRANSFORMED_ZERI_PHOTOGRAPHS));
        Timer.stop(ZeriHomogenizer.class.getCanonicalName()+".n3validation.photographs");
        log.info("FINISH: Check the validity of photographs in N3 format from Zeri in "+Timer.reportHumanFriendly(ZeriHomogenizer.class.getCanonicalName()+".n3validation.photographs"));
    }
