...
public class FrickHomogenizer implements Homogenizer{
    ...
    public void homogenizeResources() throws ETLGenericException {
        try{
            ...
            Timer.start(FrickHomogenizer.class.getCanonicalName()+".n3validation");
            Utils.N3Validator(new File(Resources.FOLDER_OUTPUT_TRANSFORMED_FRICK_ALL));
            Timer.stop(FrickHomogenizer.class.getCanonicalName()+".n3validation");
            log.info("FINISH: Check the validity of transformed resources in N3 format from FRICK in "+Timer.reportHumanFriendly(FrickHomogenizer.class.getCanonicalName()+".n3validation"));
            
            log.info("Frick Homogenization Time: "+Timer.reportHumanFriendly(FrickHomogenizer.class.getCanonicalName()));
